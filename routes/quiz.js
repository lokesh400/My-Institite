...
router.post("/create/new/streak/quiz", async (req, res) => {
    try {
      ...
      const newQuiz = new Quiz({ question, option1, option2, option3, option4, answer});
      await newQuiz.save();
...
router.post("/submit/new/streak/quiz/:quizId", async (req, res) => {
    ...
    const today = new Date().toISOString().split("T")[0];
    const lastAttemptDate = participant.lastAttemptDate ? new Date(participant.lastAttemptDate).toISOString().split("T")[0] : null;
    // Check streak logic
    if (lastAttemptDate) {
    ...
    res.json({ message: "Quiz attempt recorded", streak: participant.streak, level: participant.level, coins: participant.coins });
  } catch (error) {
    console.log(error)
  }
});
...
router.post("/submit-quiz", async (req, res) => {
    try {
      ...
      const lastAttemptDate = user.lastAttemptDate ? new Date(user.lastAttemptDate) : null;
      ...
      if (lastAttemptDate) {
        ...
        if (daysDiff < 1) {
          ...
          return res.json({ isCorrect, message: "You've already attempted a quiz today!" });
      ...
      res.json({ isCorrect, streak: user.streak, message: isCorrect ? "Correct! Streak Updated" : "Wrong Answer!" });
    } catch (error) {
